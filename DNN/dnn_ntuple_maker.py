...
OUTFILE = uproot3.recreate("/eos/user/j/jrotter/HWW_DNN_Ntuples/HWW_DNN_Ntuple_v2.root")
# FILE VERSIONING:
        ...
        for subcat_i in range(0,len(CONFIG[category]["subpaths"])):
            ...
            for f in files:
                ...
                for i, var_name in enumerate(INPUT_VARS):
                    if(any(chr.isdigit() for chr in var_name)):
                        var = var_name[:-1]
                ...
                ptl1_mask = np.array(input_vars[INPUT_VAR_INDEX["Lepton_pt[0]"]]) > 25
                ptl2_mask = np.array(input_vars[INPUT_VAR_INDEX["Lepton_pt[1]"]]) > 20
                ...
                evt_mask = mll_mask & ptll_mask & ptl1_mask & ptl2_mask & ptmiss_mask & njet_mask
